...
#
# Version 6.29: 2017-08-08
#     - Add LFD5420 alias
#     - Drop requirements for LFD420/430/440 to DISK=9 (GiB)
#     - Fix VM URL generation bug
#===============================================================================
VERSION=6.29
...
clean_cm_cache(){
    debug 1 "clean_cm_cache: $CMCACHE"
    mkdir -p "$CMCACHE"
...
cache_output() {
    local CLASS="$1"
    local CMD="$2"
    local OUTPUT="$CMCACHE/$CLASS-${CMDBASE%.sh}.output"
    debug 1 "cache_output: CLASS=$CLASS CMD=$CMD OUTPUT=$OUTPUT"
...
get_cm_file() {
    ...
    local TODIR=$3
    local PARTIAL=$4
    debug 1 "get_cm_file CLASS=$CLASS FILE=$FILE TODIR=$TODIR PARTIAL=$PARTIAL"

    local URL=$LFCM/$CLASS${FILE:+/$FILE}

    [[ -z $TODIR ]] || pushd "$TODIR" >/dev/null
    if [[ -n $NONETWORK ]] ; then
        warn "get_cm_file $FILE: No network selected."
        return 1
    elif [[ -z $USECURL ]] && which wget >/dev/null ; then
        local OPTS
        if [[ -n $FILE ]] ; then
        ...
        debug 2 "  get_cm_file: wget $WGET_CM_OPTS $OPTS $URL"
        # shellcheck disable=SC2086
        ...
        debug 2 "  get_cm_file: curl $CURL_CM_OPTS $OPTS $URL"
        # shellcheck disable=SC2086
...
try_cm_file() {
    ...
    local FILE=$2
    local TODIR=$3
    debug 1 "try_cm_file CLASS=$CLASS FILE=$FILE TODIR=$TODIR"

    local URL OPTS SIZE
    URL=$LFCM/$CLASS${FILE:+/$FILE}
    debug 2 "  try_cm_file: wget $OPTS $URL"
    SIZE=$(wget "$OPTS" "$URL" 2>&1 | awk '/Length:/ {print $2}')

    local LOCALFILE="$TODIR/$FILE"
    debug 2 "  try_cm_file: LOCALFILE=$LOCALFILE"
...
get_cm_var(){
    local KEY=$1
    local CLASS=${2# }
    debug 1 "get_cm_var: KEY=$KEY CLASS=$CLASS"

    mkdir -p "$CMCACHE"
    local CONF="${CMDBASE%.sh}.conf"
    local FILE="$CMCACHE/$CLASS-$CONF"
    debug 2 "  get_cm_var: CONF=$CONF FILE=$FILE"
    if [[ ! -e $FILE ]] ; then
...
get_course_file() {
    ...
    debug 1 "get_course_file: COURSE=$COURSE"
    local FILE VER

    if [[ -n $NONETWORK ]] ; then
        warn "get_course_file: No network selected"
        return
    fi
    ...
    if [[ -n $FILE ]] ; then
        ...
        debug 2 "   get_course_file: COURSE=$COURSE FILE=$FILE VER=$VER"
        echo "$FILE" "$VER"
    else
        debug 2 "   get_course_file: No course materials found for $COURSE"
    fi
}
...
enable_extras() {
    ...
    EXTRAS[$COURSE]="${COURSE}_%COURSE_VERSION%/"
}
...
PACKAGES[Debian-8_@docker]="-docker.io"
PACKAGES[Debian-999_@docker]="docker.io"
PACKAGES[Ubuntu-14.04_@docker]="docker.io"
PACKAGES[Ubuntu-16.04_@docker]="[Ubuntu-14.04_@docker]"
...
#-------------------------------------------------------------------------------
PACKAGES[LFD301]="@build @common @sysadm @trace cvs gparted lvm2 subversion sysstat tcpdump wireshark"
...
value() {
    ...
    LIST='${'$VAR'['$CLASS']} ${'$VAR'['${CLASS:0:3}']} $'$VAR
...
for_each_course() {
    ...
    local COURSES CLASS
    COURSES=$(list_sort "$@")
    debug 1 "for_each_course: CODE=$CODE COURSES=$COURSES"
    for CLASS in $COURSES ; do
        debug 2 "  for_each_course: eval $CODE $CLASS"
        eval "$CODE $CLASS"
    done
}
...
supported_course() {
    local CLASS=$1
    [[ -n ${TITLE[$CLASS]} ]] || warn "Unsupported course: $CLASS"
}
...
check_course() {
    local CLASS=$1
    local DESC=${TITLE[$CLASS]}
    debug 1 "check_course: CLASS=$CLASS"

    if [[ -z $DESC ]] ; then
        if DESC=$(get_cm_var TITLE "$CLASS") && [[ -n $DESC ]] ; then
            debug 2 "  check_course: Custom $CLASS -> $COURSES"
        elif [[ -n ${COURSE_ALIASES[$CLASS]} ]] ; then
            debug 2 "  check_course: Alias $CLASS"
            CLASS=${COURSE_ALIASES[$CLASS]}
            DESC=${TITLE[$CLASS]}
        fi
        debug 2 "  check_course: CLASS=$CLASS DESC=$DESC"
    fi
...
# shellcheck disable=SC2005,SC2046,SC2086
[[ -n $ALL_COURSES ]] && COURSE=$(echo $(list_sort ${!TITLE[*]}))
...
package_list() {
    ...
    debug 1 "package_list: DID=$DID DREL=$DREL CLASS=$CLASS"

    pkg_list_check

    if [[ -n ${COPYPACKAGES[$CLASS]} ]] ; then
        debug 1 "package_list: COPYPACKAGES $CLASS -> ${COPYPACKAGES[$CLASS]}"
    ...
    # shellcheck disable=SC2086
    debug 1 "package_list: Final packages for $DID-${DVER}_$CLASS:" $LIST
    echo "$LIST"
}
...
check_packages() {
    ...
    # shellcheck disable=SC2086
    [[ -n $COURSES ]] || COURSES=$(list_sort ${!TITLE[*]})
    # shellcheck disable=SC2086
    debug 1 "check_packages: COURSES="$COURSES
...
list_packages() {
    ...
    DISTS=$(distrib_list)
    # shellcheck disable=SC2086
    debug 1 "list_packages: COURSES=$COURSES PKGS=$PKGS DISTS="$DISTS

    if [[ -n $COURSES ]] ; then
        supported_course "$COURSES"
    else
        # shellcheck disable=SC2086
        COURSES=$(list_sort ${!TITLE[*]})
    fi
    for D in $COURSES; do
...
calculate_materials() {
    ...
    # shellcheck disable=SC2034
    read -r APPENDICES VER <<< $(get_course_file "$C" APPENDICES)
    # shellcheck disable=SC2034
    read -r ERRATA VER2 <<< $(get_course_file "$C" ERRATA)
    # shellcheck disable=SC2034
    read -r RESOURCES VER3 <<< $(get_course_file "$C" RESOURCES)
    # shellcheck disable=SC2034
    read -r SOLUTIONS VER4 <<< $(get_course_file "$C" SOLUTIONS)
...
json_courses() {
    local DISTS="$1" C
    # shellcheck disable=SC2086
    debug 1 "json_courses: DISTS="$DISTS
...
clear_installed() {
    if [[ -n $ALL_COURSES ]] ; then
        $DRYRUN rm -f "$RFCONFIG"/*-installed-packages.list
    else
        local CLASS FILE
        # shellcheck disable=SC2048
        for CLASS in $* ; do
            FILE="$RFCONFIG/${CLASS}-installed-packages.list"
            if [[ -f $FILE ]] ; then
                $DRYRUN rm "$FILE"
            fi
        done
...
run_extra_code() {
    ...
    local COURSE=$3
    local CODE
    debug 1 "run_extra_code: DID=$DID DREL=$DREL COURSE=$COURSE"
...
try_packages() {
    ...
    debug 1 "try_packages: ID=$ID RELEASE=$RELEASE COURSE=$(echo $COURSE) PKGLIST="$PKGLIST

    #-------------------------------------------------------------------------------
    if [[ $ID =~ Debian|Kubuntu|LinuxMint|Mint|Ubuntu|Xubuntu ]] ; then
        # shellcheck disable=SC2086
        debinstall "$COURSE" $PKGLIST
        # shellcheck disable=SC2086
    ...
    #-------------------------------------------------------------------------------
        ...
        # shellcheck disable=SC2086
        rpminstall "$COURSE" "$PKGMGR" $PKGLIST
        # shellcheck disable=SC2086
        for_each_course "run_extra_code $ID $RELEASE" $COURSE
    ...
    #-------------------------------------------------------------------------------
        ...
        # shellcheck disable=SC2086
        rpminstall "$COURSE" zypper $PKGLIST
        # shellcheck disable=SC2086
        for_each_course "run_extra_code $ID $RELEASE" $COURSE
...
rm_packages() {
    ...
    PKGLIST=$(read_installed $COURSE)
    # shellcheck disable=SC2116,SC2086
    debug 1 "rm_packages: ID=$ID RELEASE=$RELEASE COURSE=$(echo $COURSE) PKGLIST="$PKGLIST
...
setup_meta() {
    local CLASS=$1
    debug 1 "setup_meta: CLASS=$CLASS"
    ...
    for ATTR in $ATTRS ; do
        eval "case \$$ATTR in
            [0-9]*) [[ \${$ATTR[$CLASS]} -le \$$ATTR ]] || $ATTR=\"\${$ATTR[$CLASS]}\" ;;
            *) [[ -z \${$ATTR[$CLASS]} ]] || $ATTR=\"\${$ATTR[$CLASS]}\" ;;
        esac
        debug 1 \"  $ATTR=\$$ATTR (\${$ATTR[$CLASS]})\""
        if [[ $ATTR == DISTROS ]] ; then
            # shellcheck disable=SC2086
            DISTROS[$CLASS]="$(fix_distro_alias ${DISTROS[$CLASS]})"
        fi
    done
}
...
list_requirements() {
    local COURSE=$*
    local C DISTS DIST
    [[ -n $COURSE ]] && for_each_course supported_course "$COURSE"
    # shellcheck disable=SC2086
    [[ -n $COURSE ]] && COURSES="$COURSE" || COURSES=$(list_sort ${!TITLE[*]})
    debug 1 "list_requirements: COURSE=$COURSE COURSE=$COURSES"
    echo 'Courses:'
    for C in $COURSES; do
        echo "  $C:"
        list_entry TITLE "${TITLE[$C]}"
        list_entry WEBPAGE "${WEBPAGE[$C]}"
        list_entry INPERSON "${INPERSON[$C]}"
        list_entry ONSITE "${ONSITE[$C]}"
        list_entry SELFPACED "${SELFPACED[$C]}"
        list_entry MOOC "${MOOC[$C]}"
        list_entry INSTR_LED_CLASS "${INSTR_LED_CLASS[$C]}"
        list_entry SELFPACED_CLASS "${SELFPACED_CLASS[$C]}"
        list_entry PREREQ "${PREREQ[$C]}"
        list_entry OS "$(value OS $C)"
        list_entry OS_NEEDS "$(value OS_NEEDS $C)"
        list_entry ARCH "$(value ARCH $C)"
        list_entry CPUFLAGS "$(value CPUFLAGS $C)"
        list_entry CPUS "$(value CPUS $C)"
        list_entry PREFER_CPUS "$(value PREFER_CPUS $C)"
        list_entry BOGOMIPS "$(value BOGOMIPS $C)"
        list_entry RAM "$(value RAM $C)"
        list_entry DISK "$(value DISK $C)"
        list_entry BOOT "$(value BOOT $C)"
        list_entry CONFIGS "$(value CONFIGS $C)"
        list_entry INTERNET "$(value INTERNET $C)"
        list_entry VMOKAY "$(value VMOKAY $C)"
        list_entry SYMLINKS "$(value SYMLINKS $C)"
        list_entry VERSIONS "$(value VERSIONS $C)"
        list_entry INCLUDES "$(value INCLUDES $C)"
        list_entry CARDREADER "$(value CARDREADER $C)"
        list_entry NATIVELINUX "$(value NATIVELINUX $C)"
        list_entry VMOKAY "$(value VMOKAY $C)"
        list_entry DISTRO_ARCH "$(value DISTRO_ARCH $C)"
        list_entry DISTRO_DEFAULT "$(value DISTRO_DEFAULT $C)"
        list_array DISTROS "" "$(value DISTROS $C)"
        # shellcheck disable=SC2086
        list_array DISTRO_BL "" "$(value DISTRO_BL $C)"
        if [[ -z $DIST_LIST ]] ; then
            DISTS=$(distrib_list)
        else
            DISTS=$DIST_LIST
...
get_course_tarball() {
    ...
    local TODIR=$3
    local PARTIAL=n
    debug 1 "get_course_tarball: COURSE=$COURSE FILE=$FILE TODIR=$TODIR PARTIAL=$PARTIAL"

    if [[ -n $NONETWORK ]] ; then
        warn "get_course_tarball: No network selected"
        return
    fi
    ...
    if [[ -f $TODIR/$FILE ]] ; then
        ...
        warn_wait "Download course materials tarball? ($FILE)" || return
    fi
...
get_course_extras() {
    ...
    local VER=$2
    local TODIR=$3
    debug 1 "get_course_extras: COURSE=$COURSE VER=$VER TODIR=$TODIR"
    local EXTRA_FILES EXTRA_FILE

    if [[ -n $NONETWORK ]] ; then
        warn "get_course_extras: No network selected"
        return
    fi

    EXTRA_FILES=$(sed -re "s/%COURSE_VERSION%/$VER/" <<<${EXTRAS[$COURSE]})
    debug 2 "   get_course_extras: EXTRA_FILES=$EXTRA_FILES"
        ...
        for EXTRA_FILE in $EXTRA_FILES ; do
            ...
            case $EXTRA_FILE in
                */) mkdir -p "$TODIR/$EXTRA_FILE"
                    ...
                    ALLFILES=$(get_cm_file "$COURSE/$EXTRA_FILE" \
                            | awk -F\" '/<td>/ {print $8}')
                    debug 2 "  get_course_extras: ALLFILES=$ALLFILES"
                    for FILE in $ALLFILES ; do
                        if try_cm_file "$COURSE/$EXTRA_FILE" "$FILE" "$TODIR/$EXTRA_FILE" ; then
                            debug 2 "  get_course_extras: $EXTRA_FILE"
                            FILES+="$FILE "
                        fi
                    done
                ...
                *)  local DIR
                    ...
                    mkdir -p "$DIR"
                    if try_cm_file "$COURSE" "$EXTRA_FILE" "$TODIR" ; then
...
get_course_materials() {
    ...
    debug 1 "get_course_materials: COURSE=$COURSE TODIR=$TODIR"
    ...
    if [[ -n $NONETWORK ]] ; then
        warn "get_course_materials: No network selected"
        return
    fi

    local FOUND SOLUTIONS VER RESOURCES VER2
    read -r SOLUTIONS VER <<< $(get_course_file "$COURSE" SOLUTIONS)
    read -r RESOURCES VER2 <<< $(get_course_file "$COURSE" RESOURCES)
    [[ -n $VER ]] || VER=$VER2
    [[ -n $VER ]] || VER=$(get_cm_var VER "$COURSE")
    debug 2 "   get_course_materials: VER=$VER SOLUTIONS=$SOLUTIONS RESOURCES=$RESOURCES"

    debug 2 "   get_course_materials: check SOLUTIONS=$SOLUTIONS"
    if [[ -n $SOLUTIONS ]] ; then
        FOUND=1
        verbose "Get solutions for $COURSE"
        get_course_tarball "$COURSE" "$SOLUTIONS" "$TODIR"
        verbose "Check for $COURSE errata..."
        get_cm_file "$COURSE" "${COURSE}_${VER}_ERRATA.txt" "$TODIR"
    fi
    debug 2 "   get_course_materials: FOUND=$FOUND"
    debug 2 "   get_course_materials: check RESOURCES=$RESOURCES"
    if [[ -n $RESOURCES ]] ; then
        FOUND=1
        verbose "Get resources for $COURSE"
        get_course_tarball "$COURSE" "$RESOURCES" "$TODIR"
    fi
    debug 2 "   get_course_materials: FOUND=$FOUND"
    if [[ -z $FOUND ]] ; then
...
get_all_course_materials() {
    debug 1 "get_all_course_file: $*"

    if [[ -n $NONETWORK ]] ; then
        warn "get_all_course_materials: No network selected"
        return
    fi
    ...
    debug 1 "Install course materials for: $* as $CMS"
    # shellcheck disable=SC2086
